...
const app = express()
const httpServer = new HttpServer(app)      // Creo una instancia del servidor HTTP pasandole Express como parametro
const io = new IOServer(httpServer)         // Creo una instancia del servidor IO pasandole el servidor HTTP como parametro
...
io.on('connection', socket => {
  ...
  // joinChat event
  socket.on('joinChat', ({ username }) => {   // Estoy agarrando como parametro el atributo username del objeto 'data' que estoy recibiendo
    /**
     ...
     */
    users.push({
      id: socket.id,                          // asigno como ID del usuario al ID del socket 
      username,
      avatarId: Math.ceil(Math.random() * 6)  // Genero un ID de avatar random para buscar en internet
    })
